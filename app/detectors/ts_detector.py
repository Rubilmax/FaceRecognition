...
def process(frame):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > conf_threshold:   #Definition of the left-top and right-bottom corners
            ...
            cv2.rectangle(out_frame, (x1, y1), (x2, y2), (0, confidence*255, 255-((confidence-0.75)/0.75)*255), 2, 8)
            cv2.rectangle(out_frame, (x1, y1), (int(x1 + 3*(x2-x1)/6), int(y1 + (y2-y1)/10)), (0, confidence*255, 255-((confidence-0.75)/0.75)*255), -1000, 8)
